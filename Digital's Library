...
function library:NewWindow(porn, hub)
    ...
    function filter(v)
        if
            v.Name:find("Toggle") or v.Name:find("Button") or v.Name:find("Box") or v.Name:find("Text") or
                v.Name:find("Bar") and not (v:IsA("Frame") or v:IsA("UIListLayout"))
         then
            return true
        else
            return false
    ...
    Search.MouseButton1Click:Connect(
        function()
            if tog == false then
                tog = true
                ...
                tween:Create(
                    Search,
                    TweenInfo.new(0.2, Enum.EasingStyle.Sine),
                    {Rotation = Search.Rotation + 360, ImageColor3 = Color3.fromRGB(255, 255, 255)}
                ):Play()
                ...
                tween:Create(
                    Search,
                    TweenInfo.new(0.2, Enum.EasingStyle.Sine),
                    {Rotation = Search.Rotation - 360, ImageColor3 = maincolor}
                ):Play()
                Search.Image = og
            end
        end
    ...
    Hub.Position = UDim2.new(0.162211571, 0, 0.0319148935, 0)
    ...
    btnGradient_2.Color =
        ...
        ColorSequenceKeypoint.new(1.00, Color3.fromRGB(193, 116, 22))
    }
    ...
    function TabHandler:NewSection(nam)
        ...
        Element_List:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(
            function()
        ...
        TabName.MouseButton1Click:Connect(
            function()
        ...
        function Element:Bind(nam, ke, callbackontyp, callbac)
            if typeof(callbac) ~= "function" then
                warn(
                    "Element:Bind() need 4 arguments (<str> name,<enum> key,<boolean> callbackwhenconfirm,<function> callback) "
                )
            end
            local callbackontype = callbackontyp or false
            local name = tostring(nam) or "Keybind"
            local key = ke or Enum.KeyCode.F
            local callback = callbac or function()
                end
            local name = tostring(nam) or tostring(math.random(1, 5000))
        ...
        function Element:Button(name, callback)
            ...
            local function fadeoff()
                tween:Create(
                    NewButton,
                    TweenInfo.new(0.2, Enum.EasingStyle.Sine),
                    {BackgroundColor3 = Color3.fromRGB(48, 48, 48), TextColor3 = Color3.fromRGB(200, 200, 200)}
                ):Play()
                tween:Create(
                    BtnIcon,
                    TweenInfo.new(0.2, Enum.EasingStyle.Sine),
            ...
            BtnIcon.MouseButton1Click:Connect(
                function()
                    CircleClick(NewButton, Mouse.X, Mouse.Y)
                    callback()
                end
        ...
        function Element:Slider(nam, mi, ma, callbac)
            ...
            Slide.MouseButton1Down:connect(
                function()
                    down = true
                    fadeon()
                    while down and game:GetService("RunService").RenderStepped:wait() do
                        local percentage =
                            math.clamp(
                            ((Mouse.X - SliderBackground.AbsolutePosition.X) / (SliderBackground.AbsoluteSize.X)),
                            0,
                            1
                        )

                        Slide:TweenPosition(
                            UDim2.new(percentage - 0.1, 0, -1.75, 0),
                        ...
                        tween:Create(
                            Slide,
                            TweenInfo.new(0.2),
                            {Size = UDim2.new(0, 18, 0, 18), ImageColor3 = Color3.fromRGB(255, 255, 255)}
                        ):Play()
        ...
        function Element:Drop(nam, lis, cb)
            ...
            DropListLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(
                function()
            ...
            local function Close()
                ...
                tween:Create(
                    DropToggleIcon,
                    TweenInfo.new(0.2, Enum.EasingStyle.Sine),
                    {Rotation = 0, ImageColor3 = Color3.fromRGB(0, 0, 0)}
                ):Play()
                tween:Create(
                    DropList,
                    TweenInfo.new(0.2, Enum.EasingStyle.Sine),
            ...
            local function Check()
                ...
                if tog == false then
                    ...
                    tween:Create(
                        DropToggleIcon,
                        TweenInfo.new(0.2, Enum.EasingStyle.Sine),
                        {Rotation = 180, ImageColor3 = Color3.fromRGB(255, 255, 255)}
                    ):Play()
            ...
            local function add(v)
                ...
                DropListBtn.MouseButton1Click:Connect(
                    function()
                        callback(tostring(v))
                        Close()
                    end
            ...
            local function fadeon()
                ...
                for i = 1, 2 do
                    tween:Create(
                        ...
                        {Position = old + UDim2.new(0, 0, 0.2, 0)}
                    ):Play()
                    wait(0.1)
            ...
            function dropfunc:on(path)
                ...
                DropRefresh.MouseButton1Click:Connect(
                    function()
                        tween:Create(
                            DropRefresh,
                            TweenInfo.new(0.5, Enum.EasingStyle.Sine),
                            {Rotation = DropRefresh.Rotation + 180 * 4}
                        ):Play()
        ...
        function Element:Toggle(nam, callback)
            ...
            function togfunc:Set(a)
                Toggle.Image =
                    (a == true and "http://www.roblox.com/asset/?id=6031068421" or
...
print("Running Latest Library Update, v1.0.0 --Jonatan (B.A.C Owner Was Here!.)")
